    ...
    <Collapsible
      ...
      trigger={
        <div className={tournamentForm[0]}>
          <div className="text"><strong>{tournament.Acronym}</strong> &nbsp; {tournament.Name}</div>
            ...
            <Formik
              ...
              onSubmit={async (values) => {
                ...
                let submit = await fetch(
                  `/api/tournaments/me?s=${profile.UUID}&t=${tournament.RecordID}`,
                  {
                    method: "POST",
                    headers: new Headers({
                        ...
                        multipliers.map((multiplier, index) => {
                            return (
                                <div className="multiplier" key={index}>
                                    <label htmlFor="settingsMultiplier">{multiplier} Multiplier <span className="specialInfo">&nbsp;* = multiplication&nbsp; / = division</span></label>
