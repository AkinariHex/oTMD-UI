  ...
  return (
    <Formik
      ...
      onSubmit={async (values) => {
        ...
        let poolSubmit = await mappools;
        poolSubmit[values.mappoolStage][values.mapMod][
          values.mapModNumber - 1
        ] = await data;

        await supabase
          .from("teams")
          .update({
            mappools: JSON.stringify(poolSubmit),
          })
          .eq("UUID", teamID);
