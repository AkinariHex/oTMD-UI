...
import { Home2, Cup, TableDocument, Login, Logout, User, Profile2User } from "iconsax-react";
import { motion } from "framer-motion";
import NotificationsBTN from "../Buttons/NotificationsBTN";

export default function Navbar({ session, userStatus, notifications }) {
    ...
    return (
        <>
            <header>
                <object type="image/svg+xml" data="/img/otmdLOGO.svg" className="logoNavbar" onClick={() => setIsDropdownOpen(false)}/>
                <nav className="navbar" onClick={() => setIsDropdownOpen(false)}>
                ...
                <div className="userInfo">
                                {session !== null ? (
                                    <>
                                        <NotificationsBTN session={session} notifications={notifications}/>
                                        <div className="userInfoContent">
                                            ...
                                            <motion.div className="profileActions" animate={isDropdownOpen ? "open" : "closed"} variants={variants} transition={{duration: 0.2}}>
                                                ...
                                                <Link href="/teams"><div className="item" onClick={() => setIsDropdownOpen(false)}><Profile2User size="16" color="#d9e3f0"/>Teams</div></Link>
